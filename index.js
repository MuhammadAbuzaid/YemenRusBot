...
// إعداد البوت باستخدام التوكن من المتغيرات البيئية
const bot = new Telegraf(process.env.BOT_TOKEN);
const ADMIN_ID = '6151168428'; // ضع الـ ID الخاص بك هنا للإدارة
...
// تعريف شكل المستخدم في قاعدة البيانات
const UserSchema = new mongoose.Schema({ chatId: String });
const User = mongoose.model('User', UserSchema);

// وسيط (Middleware) لتسجيل كل مستخدم جديد تلقائياً
...
// ==========================================
// 2. منطقة البيانات (هنا يمكنك التعديل بسهولة)
// ==========================================
// هذا الجدول يحتوي على كل الأسئلة. لإضافة صورة لاحقاً، فقط ضع رابطها في photo
const faqData = {
...
// ==========================================
// 4. المنطق البرمجي (لا تعدل هنا إلا للضرورة)
// ==========================================
...
// الرد التلقائي على الأسئلة من القائمة
...
// ==========================================
// 5. أوامر الإدارة (Stats & Broadcast)
// ==========================================
...
bot.command('broadcast', async (ctx) => { 
    if (ctx.from.id.toString() === ADMIN_ID) { 
        ...
        for (const u of users) { 
            try { await bot.telegram.sendMessage(u.chatId, msg); await new Promise(r => setTimeout(r, 100)); } catch (e) {} 
        } 
        ctx.reply(`✅ تم الإرسال للجميع.`); 
    } 
});

// التعامل مع اقتراحات التعديل
let pendingEdits = {};
